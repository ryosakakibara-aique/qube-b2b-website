...
export default async function ProductsPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[var(--background)] text-[var(--heading)]">
      ...
      <section className="mx-auto grid min-h-117.5 w-full max-w-260 gap-8 px-6 py-20 lg:grid-cols-2 lg:px-0 lg:py-37.5">
        <div className="max-w-130">
          {/* Above the fold. The heading is the largest contentful paint element, so it takes the
              shortest and only undelayed fade in the hero cascade. */}
          <HeroReveal
            as="h1"
            index={0}
            variant="heading"
            className="text-4xl font-semibold leading-[1.05] tracking-tight lg:text-[40px] lg:leading-[1.08] lg:tracking-normal"
          >
          ...
          <HeroReveal as="div" index={1}>
            <Link
              href="#contact"
              className="cta-gradient motion-press mt-8 inline-flex h-11 items-center rounded-[var(--radius-control)] px-6 text-xs font-bold text-[var(--brand-foreground)] transition-opacity hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--focus-ring)]"
            >
              Talk to an Expert
            </Link>
          </HeroReveal>
        ...
        <HeroReveal
          as="div"
          index={2}
          className="flex items-end justify-start gap-2 pb-1 text-xs lg:justify-end"
        >
      ...
      {/* The hero visual, last in the cascade. */}
      <HeroReveal as="section" index={3} variant="visual" className="w-full pb-28">
        {result.ok ? (
